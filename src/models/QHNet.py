...
from ..dataset.utils import matrix_transform
...
class QHNet(nn.Module):
    def __init__(self,
                 ...
                 max_radius=15,
                 num_nodes=10,
                 radius_embed_dim=16):  # maximum nuclear charge (+1, i.e. 87 for up to Rn) for embeddings, can be kept at default
        ...
        self.irreps_node_embedding = f'{self.hs}x0e + {self.hs}x1e + {self.hs}x2e + {self.hs}x3e + {self.hs}x4e'
    ...
    def forward(self, data):
        data.full_edge_index = get_full_graph(data)
        ...
        _, full_edge_index, full_edge_attr, full_edge_sh, transpose_edge_index = \
            self.build_graph(data, edge_index=data.full_edge_index)
    ...
    def build_final_matrix(self, batch_data):
        final_matrix = []
        dst, src = batch_data.full_edge_index
        diagonal_matrix = batch_data["pred_hamiltonian_diagonal_blocks"]
        non_diagonal_matrix = batch_data["pred_hamiltonian_non_diagonal_blocks"]

        for graph_idx in range(batch_data.ptr.shape[0] - 1):
            matrix_block_col = []
            for src_idx in range(batch_data.ptr[graph_idx], batch_data.ptr[graph_idx+1]):
                matrix_col = []
                for dst_idx in range(batch_data.ptr[graph_idx], batch_data.ptr[graph_idx+1]):
                    if src_idx == dst_idx:
                        matrix_col.append(diagonal_matrix[src_idx].index_select(
                            -2, self.orbital_mask[batch_data.atomic_numbers[dst_idx].item()]).index_select(
                            -1, self.orbital_mask[batch_data.atomic_numbers[src_idx].item()])
                        )
                    else:
                        mask1 = (src == src_idx)
                        mask2 = (dst == dst_idx)
        ...
        for i, mat in enumerate(final_matrix):
            transed_matrix.append(
                matrix_transform(
                    mat,
                    batch_data.atomic_numbers[batch_data.ptr[i]: batch_data.ptr[i+1]].cpu().squeeze().numpy(),
                    convention = "back2pyscf")
                                )
            
        if len(transed_matrix) == 1:
            # when the batch contains only one molecular(batchsize = 1)
            transed_matrix = torch.stack(transed_matrix, dim=0)
        else:
            pass
        batch_data["pred_hamiltonian"] = transed_matrix
        return batch_data
    ...
    def split_matrix(self, data):
        diagonal_matrix, non_diagonal_matrix = \
            torch.zeros(data.atomic_numbers.shape[0], 14, 14).type(data.pos.type()).to(self.device), \
        ...
        for graph_idx in range(data.ptr.shape[0] - 1):
            slices = [0]
            for atom_idx in data.atomic_numbers[range(data.ptr[graph_idx], data.ptr[graph_idx + 1])]:
            ...
            for node_idx in range(data.ptr[graph_idx], data.ptr[graph_idx+1]):
                ...
                orb_mask = self.orbital_mask[data.atomic_numbers[node_idx].item()]
            ...
            for edge_index_idx in range(num_edges, data.edge_index.shape[1]):
                ...
                orb_mask_dst = self.orbital_mask[data.atomic_numbers[dst].item()]
                orb_mask_src = self.orbital_mask[data.atomic_numbers[src].item()]
