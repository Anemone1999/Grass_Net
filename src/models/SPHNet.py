...
class Pair_construction_layer(nn.Module):
    def __init__(self,
        ...
        # this controls the size of output matrix
        if pyscf_basis_name == 'def2-svp':
            exp_irrp = o3.Irreps("3x0e + 2x1e + 1x2e")
        elif pyscf_basis_name == 'def2-svp-nabladft':
            exp_irrp = o3.Irreps("5x0e + 4x1e + 3x2e")
        elif pyscf_basis_name == 'def2-tzvp':
            exp_irrp = o3.Irreps("5x0e + 3x1e + 2x2e + 1x3e")
        elif pyscf_basis_name == 'def2-qzvp':
            exp_irrp = o3.Irreps("7x0e + 4x1e + 3x2e + 2x3e + 1x4e")
        else:
        ...
        for name in {"hamiltonian"}:
            ## expansion here follows the order of: m=-1, 0, 1, which denotes y z x
            ## the l=1 output needs to be transformed according to (2, 0, 1) to align with x y z in pyscf
    ...
    def build_final_matrix(self,batch_data,sym_type="sym"):
        # it includes building the gt and the pred, costly, TODO: get gt in dataloader
        atom_start = 0
        atom_pair_start = 0
        rebuildfocks = []
        if sym_type == "sym":
        ...
        for idx,n_atom in enumerate(batch_data.molecule_size.reshape(-1)):
            ...
            rebuildfock = block2matrix(Z,diag,non_diag,
                                       self.mask_lin,
                                       self.conv.max_block_size,
                                       sym = sym_flag, asym=asym_flag)
        ...
        if asym_flag: 
            batch_data["pred_X"] = rebuildfocks
        else:
    ...
    def forward(self, data):
        if "pred_hamiltonian_diagonal_blocks" in data.keys():
            # For QHNet, the pred_hamiltonian_diagonal_blocks is already generated
            pass
        else:
...
class SPHNet(nn.Module):
    ...
    def set(self):
        # Just to remain the same structure with QHNet
        return
