...
def block2matrix(Z,diag,non_diag,mask_lin,max_block_size,sym = False,asym = False):
    # build matrix according to convention given by mask_lin, it is also determined by CONVENTION_DICT[basis]
    if isinstance(Z,torch.Tensor):
        ...
        ## realy cost #####
        ## construct the matrix on gpu instead of on cpu and move to gpu
        rebuild_fock = torch.zeros((n_atom,n_atom,max_block_size,max_block_size), device=Z.device)
        ## ################
        
        if sym:
            ## down
            rebuild_fock[torch.eye(n_atom)==1] = diag
            row_idx, col_idx = torch.tril_indices(n_atom, n_atom, offset=-1, device=rebuild_fock.device)
            rebuild_fock[row_idx, col_idx] = 2 * non_diag
            rebuild_fock = (rebuild_fock + torch.permute(rebuild_fock,(1,0,3,2)))/2
        elif asym:
            ## down
            rebuild_fock[torch.eye(n_atom)==1] = diag

            ## fast version to get down triangular mat
            row_idx, col_idx = torch.tril_indices(n_atom, n_atom, offset=-1, device=rebuild_fock.device)
            rebuild_fock[row_idx, col_idx] = 2 * non_diag
            ## ################
            ## realy cost #####
