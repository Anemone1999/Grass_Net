...
class DataModule(LightningDataModule):
    def __init__(self, config:DictConfig):
        ...
        self.enable_rho = self.config["enable_rho"]
    ...
    def setup(self, stage):
        ...
        if stage == "fit" or stage is None or self.train_dataset is None:
            if "qh9" in self.data_name.lower() or \
                "md17" in self.data_name.lower() or \
                "custom" in self.data_name.lower() or \
                "pubchemqc" in self.data_name.lower() or \
                "nabladft" in self.data_name.lower():
                dataset = MdbDataset(
                                     path = self.path,
                                     data_name=self.data_name,
                                     remove_init=self.config["remove_init"],
                                     enable_rho=self.enable_rho,
            ...
            if "md17" in self.data_name.lower():
                ...
                print(f"MD17 {target_mole}")
                index = torch.load(
                    os.path.join(self.index_path, f'MD17_{target_mole}.pt')
                )
            ...
            if "qh9" in self.data_name.lower() and "dynamic_geo" in self.data_name.lower():
                ...
                self.idx_val = index[1]
                self.idx_test = index[2] 
            if "pubchemqc" in self.data_name.lower():
                heavy_atom_number = self.data_name.split("_")[-1]
                index = torch.load(
                    os.path.join(self.index_path, f'processed_heavy_atom_{heavy_atom_number}.pt')
                )
                self.idx_train = index[2]
                self.idx_val = index[2]
                self.idx_test = index[2] 
            if "nabladft" in self.data_name.lower():
                index = torch.load(
                    os.path.join(self.index_path, 'nabladft_split.pt')
                )
                self.idx_train = np.array(index["train_indices"])
                self.idx_val = np.array(index["val_indices"])
                self.idx_test = np.array(index["val_indices"])

            if ("qh9" not in self.data_name.lower()) or \
               ("md17" not in self.data_name.lower()) or \
               ("pubchemqc" not in self.data_name.lower()) or \
               ("nabladft" not in self.data_name.lower()):
            ...
            if self.config["used_cache"]:
                ...
                if ("qh9" not in self.data_name.lower()) and \
                   ("md17" not in self.data_name.lower()) and \
                   ("pubchemqc" not in self.data_name.lower()) and \
                   ("nabladft" not in self.data_name.lower()):
