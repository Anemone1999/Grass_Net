...
convention_dict = {
    'pyscf_def2svp': Namespace(
        ...
        orbital_order_map={
            ...
            16: [0, 1, 2, 3, 4, 5, 6, 7], 17: [0, 1, 2, 3, 4, 5, 6, 7],
...
def matrix_transform_nabladft(matrices, atoms, convention='pyscf_def2svp'):
...
class HamiltonianDatabase:
    ...
    def __getitem__(self, idx: Union[int, List]) -> Tuple:
        ...
        if isinstance(idx, list):  # for batched data retrieval
            ...
            unpacked_data = [
                self._unpack_data_tuple(chunk) for i, chunk in enumerate(data)
            ]
    ...
    def _unpack_data_tuple(self, data) -> Tuple[np.ndarray]:
        ...
        pos = self._deblob(data[2], dtype=np.float32, shape=(N, 3)) # atom pos
        atoms = self._deblob(data[1], dtype=np.int32, shape=(N)) # atom numbers
        
        Norb = int(math.sqrt(len(data[5]) // 4))  # a single float32 is 4 bytes, we have Norb**2 of them
        Ham = self._deblob(data[5], dtype=np.float32, shape=(Norb, Norb)) # Hamiltonian
