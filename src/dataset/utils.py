...
def collate_fn_unified(long_cutoff_upper = 9, unit = 1):
    ...
    def _collate_fn(list_of_data):
        ...
        for key in list_of_data[0].keys():
            ...
            elif key in ['init_fock','h1e','mol','n_orb','cid', 'single_mol', "atom_pyscf"]:
                # we need grid coords to be numpy.array
            ...
            elif key in ["C_init", "fock_init", "overlap", "grid_weights", "grid_coords"]:
                processed[key] = [torch.tensor(list_of_data[i][key]) for i in range(bs_mol)]
            
            elif key in ['D_gt']:
                if list_of_data[0][key] is not None:
                    processed[key] = [torch.tensor(list_of_data[i][key]) for i in range(bs_mol)]
