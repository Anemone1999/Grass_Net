...
import copy
import pyscf
import pyscf.dft.gen_grid as gen_grid
...
from .debug_utils import check_orbital_orders
from .utils import matrix_transform, new_LEBEDEV_ORDER
...
def get_data_default_config(data_name):
    ...
    if ("qh9" in data_name.lower()) or \
       ("md17" in data_name.lower()) or \
       ("pubchemqc" in data_name.lower()) or \
       ("nabladft" in data_name.lower()):
...
class MdbDataset(Dataset):
    ...
    def __init__(self, path, data_name, transform=None, pre_transform=None, pre_filter=None,
                remove_init = False,
                 enable_rho = False,
                 basis = "def2-svp",
        ...
        if "nabladft" in self.data_name.lower():
            self.conv, _, self.mask,_ = get_conv_variable_lin(self.basis + "-nabladft")
        else:
    ...
    def _get_grid_info(self, single_mol):
        gen_grid.LEBEDEV_ORDER = new_LEBEDEV_ORDER
        gen_grid.LEBEDEV_NGRID = np.array(list(new_LEBEDEV_ORDER.values()))
        dft_grid = gen_grid.Grids(single_mol)
        dft_grid.level = 3
        dft_grid.build()

        grid_weights = dft_grid.weights.astype(np.float64)
        grid_coords = dft_grid.coords.astype(np.float64)
    ...
    def __getitem__(self, idx):
        ...
        with db_env.begin() as txn:
            ...
            #     if atom <= 2:
            #         num_orbitals += 5
            #     elif atom > 2 and atom <= 10:
            #         num_orbitals += 14
            #     elif atom > 10 and atom <= 18:
            #         num_orbitals += 18
            #     else:
            #         num_orbitals += 32
            ...
            if len(Ham.shape) == 1:
                num_orbitals = int(np.sqrt(Ham.shape[0]))
            else:
                num_orbitals = Ham.shape[0]
            Ham = Ham.reshape(num_orbitals, num_orbitals)
            if "md17" in self.data_name.lower():
                if "def2" not in self.path:
                    ## The origin dataset need to be transform
                    # make the orbital order same as pyscf
                    Ham = matrix_transform(Ham, atoms, 'back2pyscf')
            elif "nabladft" in self.data_name.lower():
                Ham = matrix_transform(Ham, atoms, 'back2pyscf')

            # check_orbital_orders(atoms, pos, Ham, Ham_init)
            # Ham_init = np.frombuffer(data_dict['Ham_init'], np.float64)
            if self.remove_init:
                Ham_init = Ham_init.reshape(num_orbitals, num_orbitals)
                Ham = Ham - Ham_init
        
        single_mol = pyscf.gto.Mole()
        single_mol.atom = [[atom, pos[i]] for i, atom in enumerate(atoms)]
        single_mol.basis = self.basis
        if "nabladft" in self.data_name.lower():
            single_mol.unit = "bohr"
        else:
            single_mol.unit = "ang"
        single_mol.build()
        overlap = single_mol.intor('int1e_ovlp')
        atom_pyscf = single_mol.atom

        n_orb = single_mol.nelectron // 2
        if self.enable_rho:
            grid_coords, grid_weights = self._get_grid_info(single_mol)
        else:
            grid_coords = 0
            grid_weights = 0
        ...
        return {
                ...
                "atom_pyscf": atom_pyscf,         
                'molecule_size':len(pos),
                "fock": Ham.astype(np.float64),
                "fock_init": Ham_init.astype(np.float64),
                "C_init": C_init.astype(np.float64),
                "overlap": overlap.astype(np.float64),
                "D_gt": D_gt.astype(np.float64) if D_gt is not None else D_gt,
