...
def truncated_orbitals(s, trunc_factor, num_orb):
    dim = s.size(1)
    ...
    if trunc_factor >= 1:
        ...
        target = p[:, :num_orb, :num_orb].abs()
        threshold = target.quantile(trunc_factor)
    p = torch.clamp(p, min=-threshold, max=threshold)
...
class ED_trunc_p(Function):
    ...
    def forward(ctx, input, trunc_factor=3, num_orb=-1):
        ...
        if num_orb == -1:
            num_orb = p.shape[-1]
        num_orb = torch.tensor(num_orb, device=input.device, dtype=torch.int64)
    ...
    def backward(ctx, dvals, dvecs):
        ...
        k = truncated_orbitals(eig_diag, trunc_factor, num_orb)
        ...
        grad_input = truncated_gradients(grad_input, trunc_factor=0.95)
