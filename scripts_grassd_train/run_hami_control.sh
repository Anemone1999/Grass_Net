#!/bin/bash
# Task: hami*1 only (control group — grassmann enabled but weight=0)
set -e
...
python pipelines/train.py \
  job_id=v2_hami_control \
  wandb.wandb_name=v2_hami_control \
  wandb.open=true \
  ...
  used_cache=false \
  precision=32 \
  "log_dir=/home/pepe/workbench/grassd_outputs/v2_hami_control"
